        ...
        if request.headers.get('x-requested-with') == "XMLHttpRequest":
            ...
            try:
                ...
                try:
                    ...
                    chkCart.quantity += 1
                    chkCart.save()
                    return JsonResponse({'status': 'Success', 'message': 'Increased the quantity successfully.', 'cart_counter': get_cart_counter(request), 'qty': chkCart.quantity})
                except:
                    chkCart = Cart.objects.create(user=request.user, fooditem=fooditem, quantity=1)
                    return JsonResponse({'status': 'Success', 'message': 'Added to cart successfully.', 'cart_counter': get_cart_counter(request), 'qty': chkCart.quantity})
            except:
